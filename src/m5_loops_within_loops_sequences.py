...
def run_test_integers():
    ...
    # Test 2:
    expected = [1, 2, 3, 1, 2, 3, 2]
    answer = integers([(1, 2, 3), (1, 2, 3), (2.5, 1.4, 2)])
    print('Expected is:', expected)
    print('Actual is:  ', answer)
    ...
    for k in range(len(sequence_of_sequences)):
        ...
        for j in range(len(sublist)):
            if type(sublist[j]) is int:
                list_to_return.append(sublist[j])
...
def run_test_big_letters():
    ...
    answer = big_letters([('AbCdEfGhIjKlMnOpQrStUvWxYz')])
    print('Expected is:', expected)
    print('Actual is:  ', answer)
...
def big_letters(sequence_of_sequences):
    ...
    list_to_return = ''
    for k in range(len(sequence_of_sequences)):
        sub_list = sequence_of_sequences[k]
        for j in range(len(sub_list)):
            if type(sub_list) is str:
                if sub_list[j].isupper():
                    list_to_return = list_to_return + sub_list[j]
    return list_to_return
